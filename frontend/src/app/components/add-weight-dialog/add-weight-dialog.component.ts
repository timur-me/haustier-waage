...
@Component({
  ...
  template: `
    <div
      class="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center p-4 dialog-overlay"
    >
      <div
        class="bg-white dark:bg-dark-secondary rounded-lg shadow-xl max-w-md w-full mx-auto dialog"
      >
        <div class="px-6 pt-6 pb-4">
          <h3
            class="text-lg font-medium text-gray-900 dark:text-dark-text mb-4 animate-fadeIn"
          >
            Add Weight Entry
          </h3>
          <form (ngSubmit)="submit()" class="space-y-4">
            <div class="animate-slideUp" style="animation-delay: 100ms">
              <label
                for="weight"
                class="block text-sm font-medium text-gray-700 dark:text-dark-muted mb-1"
              >
                Weight (kg)
              </label>
              <input
              ...
              <p
                *ngIf="error && !weight"
                class="mt-1 text-sm text-red-500 dark:text-red-400 animate-fadeIn"
              >
                Weight is required
              </p>
            </div>
            ...
            <div
              class="flex justify-end space-x-3 pt-4 animate-slideUp"
...
export class AddWeightDialogComponent {
  ...
  submit() {
    ...
    this.apiService
      .addWeightEntry(this.animalId, this.weight, new Date(this.date))
      .subscribe({
        next: (entry) => {
